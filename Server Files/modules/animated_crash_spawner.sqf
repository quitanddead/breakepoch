...
_guaranteedLoot			= 16;				// Guaranteed Loot Spawns
_randomizedLoot			= 16;				// Random number of loot piles as well as the guaranteed ones
...
_crashDamage			= 0.3;				// Amount of damage the heli can take before crashing (between 0.1 and 1) Lower the number and the heli can take less damage before crashing 1 damage is fully destroyed and 0.1 something like a DMR could one shot the heli
_exploRange				= 200;				// How far away from the predefined crash point should the heli start crashing
_minLootRadius			= 10;				// Minimum distance for loot to spawn from the crash site in meters
_maxLootRadius			= 15;				// Maximum distance for loot to spawn from the crash site in meters
...
if (_spawnRoll <= _spawnChance) then
{
	...
	_hint = parseText format["<t align='center' color='#61B329' shadow='2' size='1.75'>Aircraft Crash</t><br/><br/><t align='center' color='#ffffff'>An aircraft is in distress! Watch for it and go to the crash site to secure the loot.</t>"];
	...
	if(_isWater) then {
			...
			// Send Top Right message to users , requires Remote message script
			_hint = parseText format["<t align='center' color='#61B329' shadow='2' size='1.75'>Aircraft Crash</t><br/><br/><t align='center' color='#ffffff'>The aircraft has crashed into the water, no loot can be secured.</t>"];
			customRemoteMessage = ['hint', _hint];
			publicVariable "customRemoteMessage";
	...
	else
	{
		...
		_endTime = time - _startTime;
			...
			// Send Top Right message to users , requires Remote message script
			_hint = parseText format["<t align='center' color='#61B329' shadow='2' size='1.75'>Aircraft Crash</t><br/><br/><t align='center' color='#ffffff'>The aircraft has crashed, go and secure the loot!</t>"];
			customRemoteMessage = ['hint', _hint];
			publicVariable "customRemoteMessage";
		...
		waitUntil
		{
			...
			// Send Top Right message to users , requires Remote message script
			_hint = parseText format["<t align='center' color='#61B329' shadow='2' size='1.75'>Aircraft Crash</t><br/><br/><t align='center' color='#ffffff'>Survivors have secured the crash site!</t>"];
			customRemoteMessage = ['hint', _hint];
			publicVariable "customRemoteMessage";
