...
if ({_object isKindOf _x} count LOG_CFG_ISTOWABLE > 0) then{
	_object addAction [("<t color='#dddd00'>" + STR_LOG_TOW + "</t>"),"scripts\logistic\object\isSelected.sqf",[],5,false,true,"","LOG_OBJECT_ADDACTION == _target && LOG_OBJECT_TRAILER_VALID"];
	_object addAction [("<t color='#dddd00'>" + STR_LOG_UNTOW + "</t>"),"scripts\logistic\tow\detach.sqf",[],6,true,true,"","LOG_OBJECT_ADDACTION == _target && LOG_DETACH_VALID"];
};