											...
											if (!(vkc_objectID == "0" && vkc_objectUID == "0")) then {
												...
												if ((count vkc_temp_keys) > 0) then {
													...
													if (s_player_claimVehicle < 0) then {
														s_player_claimVehicle = player addAction [("<t color='#00FFFF'>" + ("Claim Vehicle") + "</t>"),"scripts\VehicleKeyChanger\VehicleKeyChanger.sqf",[vkc_cTarget,vkc_temp_keys,"0",vkc_temp_keysDisplayName,"0","0","0", vkc_claimingPrice],-1002,false,false,"",""];
													};
										...
										if (!(vkc_objectID == "0" && vkc_objectUID == "0")) then {
											...
											if (vkc_hasKey && (count vkc_temp_keys) > 1) then {
												...
												if (s_player_copyToKey < 0) then {
													s_player_copyToKey = player addAction [("<t color=""#0000FF"">" + ("Change Vehicle Key") + "</t>"),"scripts\VehicleKeyChanger\VehicleKeyChanger.sqf",[vkc_cTarget, vkc_temp_keys, vkc_carKey, vkc_temp_keysDisplayName, vkc_carKeyName, vkc_targetVehicleKey, vkc_Price, "0"],-1002,false,false,"",""];
												};
