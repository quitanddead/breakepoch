...
if (!isDedicated) then {
	...
	dayz_spaceInterrupt =			compile preprocessFileLineNumbers "dayz_code\snap_pro\dayz_spaceInterrupt.sqf";
	...
	world_surfaceNoise =		compile preprocessFileLineNumbers "dayz_code\compile\fn_surfaceNoise.sqf";
	...
	player_build =				compile preprocessFileLineNumbers "dayz_code\snap_pro\player_build.sqf";
	snap_build = 				compile preprocessFileLineNumbers "dayz_code\snap_pro\snap_build.sqf";
	...
	if (DZE_ConfigTrader) then {
		call compile preprocessFileLineNumbers "dayz_code\compile\player_traderMenuConfig.sqf";
	}else{
		call compile preprocessFileLineNumbers "\z\addons\dayz_code\compile\player_traderMenuHive.sqf";
	};
	// recent murders menu code
	call compile preprocessFileLineNumbers "dayz_code\compile\player_murderMenu.sqf";

Svr_LoadingMessage = ["Loading up server."];	// innitial message before the server ever sends a message to the client.
	"PVDZE_Z_LoadMessage" addPublicVariableEventHandler {Svr_LoadingMessage = _this select 1;};
        ...
        while { _timeOut < 12000 } do {
            ...
            if (!isNil "_display") then {
                if ( isNull _display ) then {
                 ...
                 _control1 ctrlSetText format["%1 & %2.",Svr_LoadingMessage select 0 ,dayz_loadScreenMsg];             
                 _control2 ctrlSetText format["%1",round(_timeOut*0.01)];
            };
			
	    if( Svr_LoadingMessage select 0 != "Server running")then{
		    _timeOut = 0;
		}else{
                    _timeOut = _timeOut + 1;
	    };
	...
	local_lockUnlock =			compile preprocessFileLineNumbers "dayz_code\compile\local_lockUnlock.sqf";		//When vehicle is local to unit perform locking vehicle
