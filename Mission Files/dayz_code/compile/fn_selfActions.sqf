...
_textMessage = _namePlayer + " is trying to glitch with turbo key. Ban incoming!";
	...
	_player_lockUnlock_crtl = false;
	 ...
	 if (_canDo && (speed player <= 1) && (_cursorTarget isKindOf "Plastic_Pole_EP1_DZ")) then {
		 ...
		 if (s_player_maintain_area < 0) then {
		  	s_player_maintain_area = player addAction [format["<t color='#ff0000'>%1</t>",localize "STR_EPOCH_ACTIONS_MAINTAREA"], "dayz_code\actions\maintain_area.sqf", "maintain", 5, false];
		 	s_player_maintain_area_preview = player addAction [format["<t color='#ff0000'>%1</t>",localize "STR_EPOCH_ACTIONS_MAINTPREV"], "dayz_code\actions\maintain_area.sqf", "preview", 5, false];
		 };
	} else {
	...
	*/
	...
	if((_isDestructable || _cursorTarget isKindOf "Land_DZE_WoodDoorLocked_Base" || _cursorTarget isKindOf "CinderWallDoorLocked_DZ_Base") && (DZE_Lock_Door == _ownerID)) then {
		...
		if (s_player_downgrade_build < 0) then {
			...
			s_player_downgrade_build = player addAction [format[localize "STR_EPOCH_ACTIONS_REMLOCK",_text], "dayz_code\actions\player_buildingDowngrade.sqf",_cursorTarget, -2, false, true, "",""];
		};
	} else {
		player removeAction s_player_downgrade_build;
		s_player_downgrade_build = -1;
	...
	*/
	...
	if (_isMan && !_isPZombie && _traderType in serverTraders) then {
		...
		if (s_player_parts_crtl < 0) then {
			...
			if((_traderMenu select 2) == "sbandit") then {
				_low_high = "high";
				_humanity_logic = (_humanity > -65000);
			};
			if((_traderMenu select 2) == "hero") then {
				_humanity_logic = (_humanity < 5000);
			};
			if((_traderMenu select 2) == "shero") then {
